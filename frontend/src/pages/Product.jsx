        ...
        <div className='flex-1'>
          ...
          <div className='flex flex-col gap-4 my-8'>
            ...
            <div className='flex gap-2'>
              {productData.platform.map((item, index) => (
                <button
                  onClick={() => setPlatform(item)}
                  className={`py-2 px-4 cursor-pointer ${item === platform ? 'bg-orange-400 ' : 'bg-gray-100'}`}
                  key={index}>
                  {item}
                </button>
              ))}
          ...
          <button onClick={() => addToCart(productData._id, platform)} className='bg-black text-white px-8 py-3 text-sm active:bg-gray-700 cursor-pointer'>ADD TO CART</button>
          <hr className='mt-8 sm: w-4/5' />
          <div className='text-sm text-gray-500 mt-5 flex flex-col gap-1'>
            <p>100% Official Product</p>
            <p>Cash on Delivery is not available on any product</p>
